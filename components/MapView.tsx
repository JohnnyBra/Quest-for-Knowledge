...
const MapView: React.FC<MapViewProps> = ({ mapData, player, activeEnemies, onMove, theme }) => {
  ...
  // THEME STYLES
  const getThemeStyles = (type: TileType, x: number, y: number) => {
  ...
  const renderTile = (type: TileType, x: number, y: number, isVisible: boolean) => {
    // VISUAL FOG OF WAR
    if (!isVisible) {
         return (
             <div 
             ...
             >
                <div className="absolute inset-0 bg-slate-900 border border-slate-800"></div>
                <div className="absolute inset-0 flex items-center justify-center opacity-30">
                   <Cloud size={24} className="text-slate-600 fill-slate-700 animate-pulse" />
                </div>
             </div>
         );
    }
    ...
    // Get Base Style
    const { style, className, content } = getThemeStyles(type, x, y);
        ...
        >
            ...
            >
                ...
                {mapData.tiles.map((row, y) => 
                    row.map((tile, x) => renderTile(tile, x, y, mapData.visited[y][x]))
                )}
